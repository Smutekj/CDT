...
namespace cdt
{
    ...
    {
        const size_t ix = static_cast<size_t>(std::floor((r_coord.x - m_grid_offset.x) / m_cell_size.x)) % m_cell_count.x;
        const size_t iy = static_cast<size_t>(std::floor((r_coord.y - m_grid_offset.y) / m_cell_size.y)) % m_cell_count.y;
    ...
    //! \returns x component of 2D cell coordinates
    size_t Grid::cellCoordX(const cdt::Vector2f r_coord) const { return static_cast<size_t>((r_coord.x - m_grid_offset.x) / m_cell_size.x); }

    //! \brief computes the 2D cell coordinates of cell containing the given point
    //! \param r_coord given point
    //! \returns y component of 2D cell coordinates
    size_t Grid::cellCoordY(const cdt::Vector2f r_coord) const { return static_cast<size_t>((r_coord.y - m_grid_offset.y) / m_cell_size.y); }
    ...
    {
        const auto cell_coord_x = static_cast<int>((r_coord.x - m_grid_offset.x) / m_cell_size.x);
        const auto cell_coord_y = static_cast<int>((r_coord.y - m_grid_offset.y) / m_cell_size.y);
    ...
    Grid::Grid(cdt::Vector2i n_cells, cdt::Vector2f box_size, cdt::Vector2f offset)
        : 
